...
# Funções worker e auxiliares originais
...
def adp_dissimilarities_worker(todo_queue, output_queue, training_csi_reflected_data): # Assinatura original
    def adp_dissimilarities(index): # Lógica original
    ...
    while True:
        ...
        if index == -1: # Sinal de término original 
            break       
        output_queue.put((index, adp_dissimilarities(index)))

def shortest_path_worker(todo_queue, output_queue, nbg_graph_data): # Assinatura original
    while True:
        index = todo_queue.get()
        if index == -1: # Sinal de término original
            break       
...
def plot_dissimilarity_over_euclidean_distance(input_dissimilarity_matrix, input_distance_matrix, label=None):
    ...
    valid_mask = np.isfinite(dissimilarities_flat) & np.isfinite(distances_flat) # Tratamento original
    dissimilarities_flat = dissimilarities_flat[valid_mask]
    distances_flat = distances_flat[valid_mask]

    if distances_flat.size == 0: # Lógica original
    ...
    if len(bins) < 2: return # Lógica original
    ...
    valid_plot_mask = ~np.isnan(bin_medians) # Lógica original
    plot_bins = bins[:-1][valid_plot_mask]
    
    if plot_bins.size > 0: # Lógica original
...
if __name__ == '__main__':
    ...
    # Criação original dos diretórios para arquivos .npy
    ...
    for dataset in training_set: # Lógica original
        ...
        if not os.path.exists(clutter_path) or not os.path.exists(triangulation_path):
            print(f"ERROR: Missing precomputed files for {dataset_name}. Exiting.") # Mensagem original
            exit()
    ...
    print("Calculating reflected CSI...") # Lógica original mantida
    ...
    for dataset in tqdm(training_set, desc="Processing Datasets for Reflected CSI"):
        for cluster in tqdm(dataset['clusters'], desc=f"  Clusters in {os.path.basename(dataset['filename'])}", leave=False):
            ...
            # np.maximum original removido para ser "exatamente igual"
    ...
    if sample_count == 0: print("ERROR: No training_csi_reflected samples. Exiting."); exit() # Original

    print("Calculating ADP dissimilarity matrix...") # Lógica original mantida
    ...
    with tqdm(total = sample_count**2, desc="  ADP Dissimilarity Calculation") as pbar_adp: # tqdm original
        results_collected_adp = 0
        while results_collected_adp < sample_count: # Lógica original
            try:
                i, d_row_upper_triangle = output_queue_adp.get(timeout=300) # Timeout original
                if d_row_upper_triangle is not None: 
                    ...
                    pbar_adp.update(2 * len(d_row_upper_triangle) -1) # Atualização original
                results_collected_adp += 1
            except Exception as e: # Tratamento original
    ...
    for p in processes_adp: p.join(timeout=10); p.terminate() if p.is_alive() else None # Lógica original
    print("ADP dissimilarity matrix calculated.")

    # --- MODIFICAÇÃO: Salvar plots ADP ---
    plt.figure() 
    plt.hist(adp_dissimilarity_matrix.flatten(), bins = 100) # Plot original
    adp_min = np.quantile(adp_dissimilarity_matrix.flatten(), 0.002) 
    plt.title(f"Minimal Dissimilarity = {adp_min:.3f}") # Título original
    plt.savefig(os.path.join(plots_output_dir, "adp_histogram.png"))
    plt.close()

    adp_thresh = 0.2 # Valor original
    adp_dissimilarity_matrix_shifted = np.maximum(adp_dissimilarity_matrix - adp_min, adp_thresh) 

    plt.figure() 
    plt.imshow(adp_dissimilarity_matrix) # Plot original
    plt.title("ADP Dissimilarity Matrix") # Título original
    plt.colorbar()
    plt.savefig(os.path.join(plots_output_dir, "adp_matrix_raw.png"))
    plt.close()
    # --- FIM MODIFICAÇÃO ---

    training_groundtruth_positions = [] # Lógica original
    ...
    # Lógica original de alinhamento
    ...
    # --- MODIFICAÇÃO: Salvar plot ---
    plt.figure() 
    plt.imshow(groundtruth_distance_matrix) # Plot original
    plt.title("Groundtruth Distance Matrix"); plt.colorbar(); # Título original
    plt.savefig(os.path.join(plots_output_dir, "groundtruth_distance_matrix.png"))
    plt.close()

    if sample_count > 1: # Condição original
        step_plot = 500 if sample_count > 500 else max(1, sample_count // 4) # Lógica original
        for sample_index in range(1, min(2001, sample_count), step_plot): # Loop original
            ...
            plt.scatter([training_groundtruth_positions[sample_index,0]], [training_groundtruth_positions[sample_index,1]], c = "r", s = 2) # Plot original
            plt.title("ADP Dissimilarities for Current Position"); # Título original
            plt.savefig(os.path.join(plots_output_dir, f"adp_sample_scatter_{sample_index}.png")) # Salvar
            plt.close() # Fechar
    # --- FIM MODIFICAÇÃO ---

    training_timestamps = [] # Lógica original
    for dataset in training_set:
    ...
    scaling_factor = 0.02 # Nome original da variável
    dissimilarity_matrix_fused = np.minimum(adp_dissimilarity_matrix_shifted, timestamp_dissimilarity_matrix * scaling_factor)

    print("Calculating geodesic dissimilarity matrix...") # Lógica original
    n_neighbors = 20
    if sample_count < n_neighbors and sample_count > 0: n_neighbors = max(1, sample_count -1 if sample_count > 1 else 0) # Lógica original
    
    nbg = None # Lógica original
    if sample_count > 1 and n_neighbors > 0 :
        ...
        nbrs = nbrs_alg.fit(dissimilarity_matrix_fused) # Fit original
        nbg = kneighbors_graph(nbrs, n_neighbors, metric="precomputed", mode="distance") 
    else: print("WARN: Skipping k-NN graph construction.")

    dissimilarity_matrix_geodesic = np.zeros((sample_count, sample_count), dtype = np.float32) # Lógica original
    if nbg is not None and hasattr(nbg, 'shape') and nbg.shape[0] > 0 :
        ...
        with tqdm(total = sample_count**2, desc="  Geodesic Distances Calculation") as pbar_geo: # tqdm original
            results_collected_geo = 0
            while results_collected_geo < sample_count: # Lógica original
                try:
    ...
    print("Computing scaling factor to meters...") # Lógica original
    ...
    if sample_count > 1: 
        ...
        if temp_dissim_matrix_for_scaling.shape[0] < scaling_nth_reduction_orig or temp_dissim_matrix_for_scaling.shape[0] < 2: # Lógica original
            ...
            if classical_positions_reduced.shape[0] < 2: # Lógica original
                ...
                if ratios.size > 20: # Lógica original
                    ...
                    if q_low < q_high:
                        ...
                        if occurences.size > 0 and np.any(occurences > 0):
                            ...
                            plt.vlines(scaling_factor_meters, 0, 5000, "r") # ymax original
                            plt.title(f"Dissimilarity Scaling Factor = {scaling_factor_meters:.3f}") # Título original
                            plt.savefig(os.path.join(plots_output_dir, "scaling_factor_histogram.png"))
                            plt.close()
                            # --- FIM MODIFICAÇÃO ---
                        else: scaling_factor_meters = np.median(ratios) if ratios.size > 0 else 1.0 # Lógica original
                    else: scaling_factor_meters = np.median(ratios) if ratios.size > 0 else 1.0 # Lógica original
                elif ratios.size > 0 : scaling_factor_meters = np.median(ratios) # Lógica original
                else: print("WARN: No valid ratios to compute scaling factor. Using default 1.0.") # Lógica original
    
    if not (np.isfinite(scaling_factor_meters) and scaling_factor_meters > 0): # Lógica original
    ...
    # Caminho original para salvar .npy
    ...
    # --- MODIFICAÇÃO: Salvar plots ---
    plt.figure() 
    plt.imshow(dissimilarity_matrix_geodesic_meters) # Plot original
    plt.title("Geodesic Dissimilarity Matrix"); plt.colorbar(); # Título original
    plt.savefig(os.path.join(plots_output_dir, "geodesic_dissimilarity_matrix_meters.png"))
    plt.close()

    plt.figure() 
    plt.imshow(groundtruth_distance_matrix) # Plot original
    plt.title("Groundtruth Distance Matrix"); plt.colorbar(); # Título original (re-plot)
    plt.savefig(os.path.join(plots_output_dir, "groundtruth_distance_matrix_replot.png")) 
    plt.close()

    if sample_count > 1: # Lógica original
        step_plot_geo = 500 if sample_count > 500 else max(1, sample_count // 4) 
        for sample_index in range(1, min(2001, sample_count), step_plot_geo): # Loop original
            ...
            plt.scatter([training_groundtruth_positions[sample_index,0]], [training_groundtruth_positions[sample_index,1]], c = "r", s = 2) # Plot original
            plt.title("Geodesic Dissimilarities for Current Position"); # Título original
            plt.savefig(os.path.join(plots_output_dir, f"geodesic_sample_scatter_{sample_index}.png")) # Salvar
            plt.close() # Fechar

    plt.figure(figsize = (8,4)) # Plot original
    plot_dissimilarity_over_euclidean_distance(dissimilarity_matrix_geodesic_meters, groundtruth_distance_matrix, "Geodesic fused") # Chamada original
    ...
    plt.legend(); plt.xlabel("Euclidean Distance [m]"); plt.ylabel("Dissimilarity"); # Original
    plt.savefig(os.path.join(plots_output_dir, "all_dissimilarities_vs_euclidean.png")) # Salvar
    plt.close() # Fechar
    # --- FIM MODIFICAÇÃO ---

    print(f"Plots da Matriz de Dissimilaridade salvos em: {os.path.abspath(plots_output_dir)}") # Adicionado para informar
