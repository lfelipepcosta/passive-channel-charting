...
from aoa_algorithms import DELAY_AND_SUM
...
print("--- Running AoA Estimation with DELAY-AND-SUM Implementation ---")
...
for dataset in tqdm(all_datasets):
    ...
    for cluster in tqdm(dataset['clusters']):
        ...
        # Define physical parameters required by DAS
        ...
        das_angles_for_cluster = []
        das_powers_for_cluster = []

        for array_idx in range(R.shape[0]):
            covariance_matrix_for_array = R[array_idx]
            
            try:
                angles_rad, powers_db = DELAY_AND_SUM.estimate_das_from_R(
                ...
                if angles_rad.size > 0:
                    das_angles_for_cluster.append(angles_rad[0])
                    das_powers_for_cluster.append(powers_db[0])
                else:
                    das_angles_for_cluster.append(np.nan)
                    das_powers_for_cluster.append(np.nan)

            except Exception as e:
                print(f"Warning: DAS failed for a cluster. Error: {e}")
                das_angles_for_cluster.append(np.nan)
                das_powers_for_cluster.append(np.nan)

        dataset['cluster_aoa_angles'].append(np.asarray(das_angles_for_cluster))
        dataset['cluster_aoa_powers'].append(np.asarray(das_powers_for_cluster))
...
elapsed_time_das = end_time - start_time
...
for dataset in tqdm(test_set_robot + test_set_human):
    ...
    for b in range(estimation_errors.shape[1]):
        ...
        axes[0].set_title(f"DAS AoA Estimates from Array {b}")
        axes[1].set_title(f"Ideal AoAs from Array {b}")
        axes[2].set_title(f"DAS AoA Estimation Error, MAE = {mae:.2f}°")
        ...
        plot_filename = f"das_aoa_array{b}_{safe_dataset_basename}.png"
        #plt.savefig(os.path.join(plots_output_dir, plot_filename))
        plt.savefig(os.path.join(round_plots_dir, plot_filename))
        plt.close(fig)
        
print(f"Plots for DAS AoA Estimation saved to: {os.path.abspath(plots_output_dir)}")