...
def plot_colorized(positions, groundtruth_positions, suptitle = None, title = None, show = True, alpha = 1.0, outfile = None, xlim=None, ylim=None):
    """
    Creates a 2D scatter plot where points are colored based on their groundtruth position.
    This version accepts optional xlim and ylim arguments for dynamic axis limits,
    and calculates a smart default if they are not provided.
    """
    # Generate RGB colors for each datapoint
    center_point = np.zeros(2, dtype=np.float32)
    if groundtruth_positions.shape[0] > 0:
    ...
    NormalizeData = lambda in_data: (in_data - np.min(in_data)) / (np.max(in_data) - np.min(in_data)) if (np.max(in_data) - np.min(in_data)) > 0 else in_data
    ...
    if outfile is None:
        if suptitle is not None: plt.suptitle(suptitle, fontsize=14)
        ...
        # Logic to set plot limits
        if xlim is not None and ylim is not None:
            # Priority 1: Use the limits if they are explicitly passed.
            plt.xlim(xlim)
            plt.ylim(ylim)
        else:
            # Priority 2 (Fallback): Calculate smart default limits.
            if positions.shape[0] > 0 and groundtruth_positions.shape[0] > 0:
                all_x = np.concatenate([positions[:, 0], groundtruth_positions[:, 0]])
                all_y = np.concatenate([positions[:, 1], groundtruth_positions[:, 1]])
                default_xlim = (np.min(all_x) - 0.5, np.max(all_x) + 0.5)
                default_ylim = (np.min(all_y) - 0.5, np.max(all_y) + 0.5)
...
def plot_error_ecdf(estimated_positions, groundtruth_positions, maxerr = 1.2, outfile = None):
    """
    Computes and plots the Empirical Cumulative Distribution Function (ECDF) of the localization errors.
    """
    errorvectors = groundtruth_positions - estimated_positions
    errors = np.sqrt(errorvectors[:,0]**2 + errorvectors[:,1]**2)
